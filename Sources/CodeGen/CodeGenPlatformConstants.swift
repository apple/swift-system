...
import Darwin
internal let platformName = "Darwin"
private let platformCondition = "os(macOS) || os(iOS) || os(watchOS) || os(tvOS)"
#elseif os(Linux)
import Glibc
internal let platformName = "Linux"
private let platformCondition = "os(Linux)"
#elseif os(Windows)
import ucrt
...
public let packagePath = FilePath(#filePath)
  .removingLastComponent()
  .removingLastComponent()
  .removingLastComponent()

public let platformConstantsPath = packagePath.appending(
  "Sources/System/autogenerated/\(platformName)PlatformConstants.swift")

public let platformTestsPath = packagePath.appending(
  "Tests/SystemTests/autogenerated/\(platformName)PlatformConstantsTests.swift")

extension String: Error {}

public func populatePlatformConstants(
  constantsPath: FilePath, testsPath: FilePath
) throws {
  ...
  let outputFD = try FileDescriptor.open(
    ...
    permissions: [.otherRead, .ownerReadWrite, .groupRead])
  try outputFD.writeAll(constantsFile.utf8)
  ...
  let testFD = try FileDescriptor.open(
    testsPath,
    .writeOnly,
    options: [.create, .truncate],
    permissions: [.otherRead, .ownerReadWrite, .groupRead])
  try testFD.writeAll(testFile.utf8)
}
