...
| swift-system 1.4.0 through 1.6.x | >= Swift 5.9  | >= Xcode 15.0 |
| swift-system 1.7.0 through 1.8.x | >= Swift 6.1 | >= Xcode 16.3 |
...
| ----------------------- | ----------- |
| swift-system 1.3.x | release/1.3.0 |
| swift-system 1.4.x | release/1.4.0 |
| swift-system 1.5.x | release/1.5.0 |
| swift-system 1.6.x | release/1.6.0 |
| swift-system 1.7.x | release/1.7.0 |
| swift-system 1.8.x | release/1.8.x |
| swift-system 1.9.x (unreleased) |  |
...
`release/1.7.0` → `release/1.8.x` → `main`

For example, anything landing on `release/1.6.0` will eventually appear on `release/1.7.0`, then `release/1.8.x` and finally `main` as well. There is no need to file standalone PRs for each release line. (Change propagation currently requires manual work -- it is performed by project maintainers.)
